...
def dropBetween(df, col_label_start, col_label_end):
    ...
    return df.drop(col_labels_to_drop, axis=1)
#drop all columns apart from ones in keep_cols
...
def process_DRIFF_H(day_num):
    ...
    #drop redundant data
    df = dropAllBut(dfm, ['SEQN', 'DR1IFDCD', 'DR1IGRMS'])
...
def process_DRTOT_H(day_num):
    ...
    with open(file_path, 'rb') as f:
        dfm = xport.to_dataframe(f)
    df = dfm
    df = df.dropBetween(df [WTDRD, DRTKCAL])
...
def process_DSQTOT_H():
    #setup some var names based on day
    DSQTOT_H = 'DSQTOT_H'.format(day_num)
...
def aggregate():
    #DR2IFF_H subset of DR1IFF_H subset of DR1TOT_H eq. set of DR2TOT_H
    ...
    labels = list(itertools.chain.from_iterable([file.as_matrix() for file in files]))
