...
from cvae_m_data_loaders import NHANES, mkdir_p, setup_data_loaders
...
class CVAE(nn.Module):
    ...
    def model(self, y, m, x):
        ...
        with pyro.iarange("data", x.size(0)):
            ...
            pyro.sample("obs",
                        dist.Bernoulli(loc_measurements).mask(m.reshape(-1,self.y_dim)).independent(1),
    ...
    def sim_measurements(self, x, stepQty):
        ...
        yout = torch.zeros(x.size()[0],self.y_dim)
        #print(x)
...
def get_accuracy(data_loader, classifier_fn, batch_size):
    ...
    accuracy = (accurate_preds * 1.0) / tot_preds
    return accuracy
